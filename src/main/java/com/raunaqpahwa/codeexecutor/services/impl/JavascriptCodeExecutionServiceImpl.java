...
public class JavascriptCodeExecutionServiceImpl extends CodeExecutionService {
    ...
    private static final String CONTAINER_TYPE = DockerContainer.Language.JAVASCRIPT.name();
    ...
    public ExecCreateCmdResponse createExecCmd(String containerId, String rawCode) {
        var createFileCmd = String.format("echo \"%s\" > %s", rawCode, Constants.JAVASCRIPT_FILE);
        var runFileCmd = String.format("node %s", Constants.JAVASCRIPT_FILE);
